<template>
  <q-page class="flex flex-center midia">
    <div
      v-if="itemAtual.midias.length === 0"
      class="no-data"
    >
      Nenhum imagem foi encontrada.
    </div>
    <div
      v-show="itemAtual.midias.length > 0"
      style="width:100%"
    >
      <q-carousel
        @input="currentIndex"
      ...
      >
        <q-carousel-slide
          ...
          v-for="(item, index) in itemAtual.midias"
    ...
    <q-modal
      @show="onModalShowDescricao"
      class="modal-dark"
      v-model="modalMidiaDescricao"
      minimized
      position="top"
      :content-css="{minWidth: '100vw'}"
    >
      <div style="padding: 20px">
        <div class="q-display-1 q-mb-md">Editar descrição</div>
        <q-input
          ref="descricao"
          v-model="midiaDescricao"
          type="textarea"
          dark
          color="white"
